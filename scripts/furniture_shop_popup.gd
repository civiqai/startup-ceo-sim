...
func _build_item_card(item: Dictionary) -> PanelContainer:
	...
	if is_locked:
		...
		placed_label.add_theme_font_size_override("font_size", 18)
...
func _show_placement_prompt(item_id: String) -> void:
	...
	later_btn.add_theme_color_override("font_color", COLOR_TEXT_WHITE)
...
func _update_category_buttons() -> void:
	for cat_id in _category_buttons:
		...
		var is_active := cat_id == _current_category
