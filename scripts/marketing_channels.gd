extends RefCounted
## マーケティングチャネル定義（8チャネル）
## コスト帯: 30 → 100 → 300 → 500 → 1000 → 2000 → 5000 → 10000万円
...
const CHANNELS := [
	...
	},
	{
		...
		"name": "スーパーボウルCM",
		"icon": "🏈",
		"description": "世界最大の広告枠。1億円の究極の賭け。成功すれば歴史に残る。",
...
static func apply_effect(channel_id: String, gs) -> Dictionary:
	...
	# 総合倍率
	var effectiveness: float = (1.0 + skill_bonus * 0.1) * cmo_multiplier
	...
	match channel_id:
		...
		"truck_ad":
			# 1000万: 話題性でブランド急上昇 + ユーザーもそこそこ
			var brand_roll := randi_range(8, 28)
			brand_gained = int(brand_roll * effectiveness)
			users_gained = int(randi_range(150, 600) * product_factor * effectiveness)
		...
		"superbowl_cm":
			...
			if sb_roll <= 30:
				users_gained = int(randi_range(15000, 30000) * product_factor * effectiveness)
				brand_gained = int(randi_range(35, 50) * effectiveness)
				reputation_gained = int(randi_range(20, 35) * effectiveness)
				morale_gained = int(randi_range(10, 20) * effectiveness)
				narrative = "スーパーボウルCMが世界中でバイラル化！歴代ベストCMランキング入りし、歴史に名を刻んだ！"
				rating = "大成功"
			elif sb_roll <= 65:
				users_gained = int(randi_range(5000, 12000) * product_factor * effectiveness)
				brand_gained = int(randi_range(15, 30) * effectiveness)
				reputation_gained = int(randi_range(10, 18) * effectiveness)
				morale_gained = int(randi_range(5, 10) * effectiveness)
				narrative = "スーパーボウルCM、好評を博す。世界的な知名度を獲得した。"
				rating = "成功"
			else:
				users_gained = int(randi_range(1000, 3000) * product_factor * effectiveness)
				brand_gained = int(randi_range(5, 12) * effectiveness)
				reputation_gained = int(randi_range(2, 6) * effectiveness)
				narrative = "スーパーボウルCMは不発…「よくわからなかった」との声が。1億円が重くのしかかる。"
				rating = "いまいち"
